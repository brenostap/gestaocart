...
- 🔨 ⭐ **Origem do cliente = loja (ago/2026)**: o time passou a marcar cart/urban na origem do cliente, mas (1) **não existe catálogo de origens** no banco — só `clientes.origem_id`, sem nome; falta sincronizar a lista de origens da FoneNinja; (2) origem é atributo do **cliente**, não da venda — o sync precisa **congelar** a origem na venda (coluna nova em `vendas`, gravada no `upsertVenda`), senão editar o cadastro do cliente muda a loja de vendas antigas. Hoje 11 de 2.134 clientes (0,5%) compraram nas duas lojas. Ver `docs/REGISTRO-VENDA-2026-08.md`.
- 💡 Gravar o **cadastrador** numa coluna de `vendas` (hoje só em `contas.raw->cadastrador`, garimpado por jsonb path a cada carga) — se o login virar a fonte oficial do atendente.
