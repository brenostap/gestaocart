...
- 🔨 ⭐ **Trocas detalhadas** (jul/2026): captura dos aparelhos que ENTRAM na troca (modelo/IMEI/valor). Tabela `venda_trocas` já existe (`raw` jsonb defensivo) e o app **já consome** via `_trocas` (data.js) + ficha (render.js, bloco Upgrade). ⚠️ **CORREÇÃO (30/jul/2026):** ao contrário do que esta nota dizia antes, o `salvarTrocasVenda()` **nunca existiu** no `phonecar-sync` — só o total (`upgrade_valor/qtd`) era salvo, por isso `venda_trocas` estava zerada. **Patch escrito e commitado em 30/jul** (sync.js): `salvarTrocasVenda()` grava `upgrade.produtos[]` + `raw`, chamado no `upsertVenda`; o incremental (janela de 7 dias) pega vendas novas/editadas sozinho. **Pendente**: (1) backfill dos ~1.232 históricos — Actions → Run workflow → `backfill_trocas: true`; (2) **validar o mapeamento** no `venda_trocas.raw` da 1ª rodada (`titulo/imei_1/valor` best-effort; se vier `null` com dado no `raw`, ajustar o nome no `salvarTrocasVenda`, sem mexer na tabela).
- 🔨 ⭐ **Itens de compra** (30/jul/2026): `compra_produtos` estava **parada desde ~28/mar/2026** — o `syncCompras` só gravava o cabeçalho; os itens vieram de 1 backfill único. **Patch escrito e commitado em 30/jul** (sync.js): `salvarProdutosCompra()` + busca `/compras/:id` no `syncCompras` (só das compras novas). **Pendente**: backfill do histórico — Actions → Run workflow → `backfill_compras: true` (idempotente, acha os buracos sozinho: abr→jul + fim de março). Flags no `sync.yml`.
